...
const expectRollupsToMatchRawAggregation = async (params: {
  ...
  const rollupValues =
    params.level === '30 minutes'
      ? await queryAggregatedRollups({
  ...
  rollupValues.forEach((rollupValue, index) => {
    ...
    const rawTimestamp =
      params.level === '30 minutes'
        ? rawValue.timestamp
        : formatInTimeZone(new Date(rawValue.timestamp), params.timezone, 'yyyy-MM-dd');

    expect(rollupValue.timestamp).toBe(rawTimestamp);
